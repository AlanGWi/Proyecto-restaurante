...
router.post('/addMenu', upload.single('imagen'),authorize(['MESERO','ADMIN']), MenuController.addMenu); // usar upload.single() para procesar la imagen

// Actualizar un menu existente
router.put('/menus/:id', upload.single('imagen'),authorize(['MESERO','ADMIN']), MenuController.updateMenu);
...
router.delete("/:id",authorize(['MESERO','ADMIN']), MenuController.deleteMenu);
